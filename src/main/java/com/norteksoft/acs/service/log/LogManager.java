...
public class LogManager {
	...
	public Page<Log> getAllLog(Page<Log> page,Long sysId){
		...
		sql.append(" order by l.createdTime desc");
	...
	private Object[] getLogSqlPrmts(Map<OperatorType, List<OperatorType>> authMap, StringBuilder sql, List<Object> prmts){
		...
		for(OperatorType peratorType:getOperatorTypes()){
			List<OperatorType> auths = authMap.get(peratorType);
			for(int i=0;i<auths.size();i++){
				if(StringUtils.isNotEmpty(condition.toString())){
					condition.append(" or ");
				}
				condition.append("l.operatorType=?");
	...
	public static List<OperatorType> getOperatorTypes(){
		...
		if(ContextUtils.isSystemAdmin()){
			operatorTypes.add(OperatorType.SYSTEM_ADMIN);
		}
		if(ContextUtils.isSecurityAdmin()){
			operatorTypes.add(OperatorType.SECURITY_ADMIN);
		}
		if(ContextUtils.isAuditAdmin()){
			operatorTypes.add(OperatorType.AUDIT_ADMIN);
		}
		if(operatorTypes.size()==0){
			operatorTypes.add(OperatorType.COMMON_USER);
		}
	...
	public String deleteSysLogs(String sysIds){
		...
		String successfulDeleteHint = Struts2Utils.getText("log.successful.delete");//个删除成功,
		String hint = Struts2Utils.getText("log.not.delete.within.the.retention.time");//个在保留时间内未删除。
		result.append(successNum).append(" ").append(successfulDeleteHint).append(sIds.length-successNum).append(" ").append(hint);
	...
	public String deleteAllSysLog(String sysId){
		...
		StringBuilder result=new StringBuilder();
		result.append(successNum).append(" ").append(Struts2Utils.getText("log.successful.delete")).append(" ").append(sLogs.size()-successNum).append(" ").append(Struts2Utils.getText("log.not.delete.within.the.retention.time"));
		return result.toString();
	}
	/**
	 * 是否可以删除日志
	...
	public String deleteloginUserLogAllByCompanyId(String loginLogIds, Long companyId){
		...
		StringBuilder result=new StringBuilder();
		result.append(successNum).append(" ").append(Struts2Utils.getText("log.successful.delete")).append(" ").append(ids.length-successNum).append(" ").append(Struts2Utils.getText("log.not.delete.within.the.retention.time"));
		return result.toString();
	}
	...
	public String deleteAllLoginUserLog(){
		...
		StringBuilder result=new StringBuilder();
		result.append(successNum).append(" ").append(Struts2Utils.getText("log.successful.delete")).append(" ").append(luls.size()-successNum).append(" ").append(Struts2Utils.getText("log.not.delete.within.the.retention.time"));
		return result.toString();
	}
...
/*	public Map<String,String> lookLog(Long id) throws DocumentException{
	...
	public Page<Log> search(Page<Log> page,Long systemId) {
		 StringBuilder sql = new StringBuilder("from Log l where l.companyId=? and l.systemId=? and l.deleted=?");
		 List<Object> prmts = new ArrayList<Object>();
		 prmts.add(ContextUtils.getCompanyId());
		 prmts.add(systemId);
		 ...
		 Object[] prmt = getSystemLogSqlPrmts(sql, prmts);
		 sql.append(" order by l.createdTime desc");
		 return logDao.searchPageByHql(page, sql.toString(), prmt);
	}

	public Page<LoginLog> searchLoginLog(Page<LoginLog> page) {
		StringBuilder sql = new StringBuilder("from LoginLog l where l.companyId=? and l.deleted=? ");
		 List<Object> prmts = new ArrayList<Object>();
		 prmts.add(ContextUtils.getCompanyId());
		 prmts.add(false);
		 Object[] prmt = getSystemLogSqlPrmts(sql, prmts);
		 sql.append(" order by l.loginTime desc");
		 return loginUserLogDao.searchPageByHql(page, sql.toString(), prmt);
	}
