...
<head>
		...
		if($("#prems04").attr("checked")){//如果登录安全设置中选择【锁定用户】，则【锁定时间(分钟)】必填
			if($("#lockTime").val()==""){
				...
				if(errorlables.length<=0){
					$("#lockTimeInput").append('<label  class="error">'+iMatrixMessage['common.required']+'</label>');//必填
				}
			}else{
				saveValue();
			}
	...
	function saveValue(){
		if($('input[name="resetPassword"]:checked').length>0){
			$("#resetPassword").attr("value","yes");
		}else{
			$("#resetPassword").attr("value","no");
		}
		$("#inputForm").submit();
	}
	...
	$(document).ready(function(){
		...
		if(ms=="ok"){
			$("#mse").attr("value","");
			iMatrix.alert('<s:text name="securitSet.success"/>');
		}
		getContentHeight();
	});
		...
		<aa:zone name="acs_content">
			...
			<div id="opt-content">
				<form id="inputForm" name="inputForm" action="${acsCtx}/syssetting/security-set-save.action" method="post">
		               ...
		               <tr>
		                  ...
		                  <td>
		                   <s:if test="failType=='VALIDATE_CODE'">
		                       <input  type="radio" id="prems03" name="failType" value="VALIDATE_CODE" checked="checked" onclick="lockUserClick('VALIDATE_CODE');"/><s:text name="securitySet.displayVerificationcode"/><!-- 显示验证码 -->
		                   </s:if><s:else>
		                   	   <input  type="radio" id="prems03" name="failType" value="VALIDATE_CODE" onclick="lockUserClick('VALIDATE_CODE');"/><s:text name="securitySet.displayVerificationcode"/><!-- 显示验证码 -->
		                   </s:else>
		                   <s:if test="failType=='LOCK_USER'">
		                      	<input  type="radio" id="prems04" name="failType" value="LOCK_USER"  checked="checked" onclick="lockUserClick('LOCK_USER');"/><s:text name="securitySet.lockUser"/><!-- 锁定用户 -->
		                      </s:if>
		                      <s:else>
		                     	<input  type="radio" id="prems04" name="failType" value="LOCK_USER"  onclick="lockUserClick('LOCK_USER');"/><s:text name="securitySet.lockUser"/><!-- 锁定用户 -->
		                      </s:else>
		                  </td>
		               </tr>
		              </table>
					...
					<div style="height: 10px;"></div>
					<FIELDSET><legend><s:text name="securitySet.auditInformationSet"/></legend><!-- 审计信息设置 -->
					<table class="form_table1">
				     <tr>
		             <td style="width: 170px;">
		                	<s:text name="securitySet.logSaveTime"/><!-- 日志保留时间 -->
		             </td>
		             <td >
			    ...
			    <div style="height: 10px;"></div>
				<FIELDSET><legend><s:text name="securitySet.systemToResetThePassword"/><!--用户第一次登录系统是否重新设置密码 --></legend>
					<table class="form_table1">
				        <tr>
				            <td style="width: 170px;"> <s:text name="securitySet.resetPassword"/><!--重新设置密码 --></td>
				            <td >
			                	<input type="checkbox" id="resetPassword" name="resetPassword" <s:if test="'yes'==resetPassword">checked="checked"</s:if> value="" /> 
				            </td>
				        </tr>
					</table>
			    </FIELDSET>
			    <div style="height: 10px;"></div>
				<FIELDSET><legend><s:text name="securitySet.systemDefaultLanguage"/><!--系统默认语言 --></legend>
					<table class="form_table1">
				        <tr>
				            <td style="width: 170px;"> <s:text name="securitySet.systemDefaultLanguage"/><!--系统默认语言 --></td>
				            <td >
				            	<select id="systemDefaultLanguage" name="systemDefaultLanguage">
				            	<s:iterator value="@com.norteksoft.acs.base.enumeration.SystemDefaultLanguage@values()" var="systemDefaultLanguageVar">
				            	<option <s:if test="#systemDefaultLanguageVar==systemDefaultLanguage">selected="selected"</s:if> value="${systemDefaultLanguageVar}"><s:text name="%{code}"></s:text></option>
				            	</s:iterator>
				            	</select>
				            </td>
				        </tr>
